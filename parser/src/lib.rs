...
fn parse_number(inp: &str) -> IResult<&str, JsonValue> {
    map(tuple((
        ...
        opt(parse_fraction),
        opt(parse_exponent))),
        |(n, f, exp)| JsonValue::Number(Number(n, f, exp)))(inp)
}
...
fn parse_integer(inp: &str) -> IResult<&str, i32> {
    ...
    let parser = map(alt((
        map(
            pair(parse_onenine, parse_digits),
            |(leading, rest)| {
...
fn parse_digit(inp: &str) -> IResult<&str, u32> {
    alt((map(char('0'), |c| c.to_digit(10).unwrap()),
         parse_onenine
    ))(inp)
}
...
mod test {
    ...
    fn test_parse_hex() {
        ...
        assert_eq!(Err(Err::Error(error_position!("KLM", nom::error::ErrorKind::OneOf))), parse_hex("KLM"));
    }

    #[test]
    fn test_parse_number() {
        assert_eq!(Ok(("", JsonValue::Number(Number(56172, None, None)))),
                   parse_number("56172"));
        assert_eq!(Ok(("", JsonValue::Number(Number(123, Some(Fraction(98)), None)))),
                   parse_number("123.98"));
        assert_eq!(Ok(("", JsonValue::Number(Number(167, None, Some(Exponent(Sign::Minus, 12)))))),
                   parse_number("167e-12"));
        assert_eq!(Ok(("", JsonValue::Number(Number(1, Some(Fraction(2)), Some(Exponent(Sign::Plus, 3)))))),
                   parse_number("1.2e+3"));
        assert_eq!(Err(Err::Error(error_position!("notanumber", nom::error::ErrorKind::OneOf))), parse_number("notanumber"));
    }
    ...
    fn test_parse_integer() {
        ...
        assert_eq!(Err(Err::Error(error_position!("notaninteger", nom::error::ErrorKind::Many1))), parse_digits("notaninteger"));
    }
    ...
    fn test_parse_digit() {
        ...
        assert_eq!(Err(Err::Error(error_position!("words", nom::error::ErrorKind::OneOf))), parse_digit("words"));
    }
    ...
    fn test_parse_fraction() {
        ...
        assert_eq!(Err(Err::Error(error_position!("not_frac", nom::error::ErrorKind::Char))), parse_fraction("not_frac"));
        assert_eq!(Err(Err::Error(error_position!("", nom::error::ErrorKind::Many1))), parse_fraction("."));
    }

    #[test]
    fn test_parse_exponent() {
        assert_eq!(Ok(("", Exponent(Sign::Plus, 36))), parse_exponent("E+36"));
        assert_eq!(Ok(("A", Exponent(Sign::Minus, 2222))), parse_exponent("e-2222A"));
        assert_eq!(Err(Err::Error(error_position!("A+36", nom::error::ErrorKind::Char))), parse_exponent("A+36"));
        assert_eq!(Err(Err::Error(error_position!("*36", nom::error::ErrorKind::Char))), parse_exponent("e*36"));
        assert_eq!(Err(Err::Error(error_position!("notadigit", nom::error::ErrorKind::Many1))), parse_exponent("e-notadigit"));
    }
